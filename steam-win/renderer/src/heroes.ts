...
export const HEROES: HeroDef[] = [
  ...
  },
  {
    ...
    skillName: '格網倍乘',
    skillDetail:
      '盤面當格網看：每一顆已確定的雷，火力％裡單獨計加權。數「你已放下的數字」有幾格與它緊鄰——0 或 1 格當 1；之後依 2→4→6→8 階梯加權，單顆雷上限 8。',
    combatSkills: [
      {
        name: '格網倍乘',
        hudIcon: 'grid',
        detail:
          '每顆已確定的雷：幾格已佈數字與它緊鄰，加權 0～1 格→1；2／3／4／5+ 格→2／4／6／8，封頂 8。',
      },
    ],
    missionMapHook:
    ...
    background: [
      ...
      '裂隙周邊的空間陷阱會拐人視線；她最恨「感覺對了」這種賭法。參謀台把她的格網倍乘寫進火力公式：同一雷旁邊每多一格已佈數字，權重沿 1→2→4→6→8 爬升，單顆封頂 8，直到格網讀數收斂。檔案註記：話少、筆尖快；誰踩進扭曲帶，她就用下一個標把誰拽回來。',
    ],
...
/** 火力％分子：每顆已揭示雷依「幾格已佈數字與其邏輯相鄰」加權；賽琳娜格網倍乘（1→2→4→6→8 封頂），其餘 min(n,2) 封頂。 */
