...
export const HEROES: HeroDef[] = [
  {
    ...
    barrage: {
      opening: [
        '（吞口水）電報來了……先點數字、再點格，手指別滑……',
        '（呼吸沈重）一格一格來——護目鏡外藍光又閃了，照表讀，別亂。',
      ],
      lastTen: [
        '十秒了欸……快把退路弄開，別堵在雷前面！',
        '倒數在咬人——挑最有把握那格，快！',
  ...
  },
  {
    ...
    missionBriefByLevelId: {
      ...
      // 第 5 章：斷線封鎖
      '33': '方斷線北邊的訊號全糊了。小明，校準一條線，就代表我們要親手掐斷另一條線。',
      '34': '方廊道兩側的方波出現了大面積死寂。別漂移，對齊節點，我們在用時間去換人命。',
      '35': '方偵測到兩處求救，但頻寬不夠同時並聯。選一條吧，工匠。沒被選到的那一條，安靜了。',
      '36': '方十字路口的阻抗在飆升。一格鬆，後面求救的訊號全反射。你手上的紅筆，別發抖。',
      '37': '方據點點亮了，但撤離底線正在崩塌。窄帶通訊裡……我聽見了後方撤退平民的哭聲。',
      '38': '六地形切換，六角殘陣！不要用單點信仰去猜座標，兩側同時驗證，死線要咬上來了！',
      '39': '六補給週期斷了，頻譜急需補電。節點傳來的不是數據，是那邊弟兄最後的對時聲。',
      '40': '六核心鎖死。小明，你用紅筆圈下了這條防線。這張盾……是用另外四個區段的沉默換來的。',
      // 第 6 章：深海要塞
      '41': '方雙錨定位出現自激。密碼鋼印有重疊痕跡……這不是巧合，有人動過我的後台。',
      '42': '六廢雷補入的頻率變快了。有人在用海星陣的地形代碼，向外發送我們的脈衝。',
      '43': '方縱深雷區被假信號塞滿了。那個內鬼正在利用級聯放大，想用廢物把我們窒息在艙底。',
      '44': '方錨A和錨B的數據在打架。信任的焊縫已經斷了。小明……你現在，也開始懷疑我了吧？',
      '45': '六內鬼的跳頻信號在蟹鉗堡的中繼點自激了！抓住這個頻譜，逼他露馬腳！',
      '46': '方兩點拉力達到極限。這不是噬星者的戰術，這是人類工兵的手筆。對方……很懂你。',
      '47': '六深度佈雷不是堆料。每下一步，廢雷就在周圍咬死一格。別慌，把內鬼的偽裝扒下來。',
      '48': '方兩個峰疊成了一個主峰。座標鎖定了，發射洩密電碼的終端機……就在我們前方的深醫艙！',
      // 第 7 章：雜訊審判
      '49': '方天線塔在審訊帶自激。電碼在跳——監聽台聽著，先鎖讀值再下格。',
      '50': '方雷達碟掃過假峰值。別信第一個數字，那是給審判席看的煙霧。',
      '51': '方雙波疊影裡藏密鑰。小明，幫我把清白波形釘出來。',
      '52': '方電磁方陣在餵假指令。每一手都要能在頻譜上自證。',
      '53': '方干擾錐頂到了。廢雷混進來——別讓雜訊替你認罪。',
      '54': '六磁暴環收口。據點空了，審訊官的筆就會落下。',
      '55': '六稜鏡把訊號折成謊言。兩側交叉驗證，別讓艾達替假碼背鍋。',
      '56': '六火箭筒座標鎖死。密鑰最後一截在這——錯一格，她出不去這間監聽室。',
      // 第 8 章：無名代價
      '57': '方餘韻盤上第一顆定時雷在走。無名部隊在頻道外等你的排程表。',
      '58': '方引爆單核——爆心底下是礦區帳篷。先拆引信，再問良心。',
      '59': '方雙雷狹峰線，兩顆同時跳。效率和人命，你選哪顆先救。',
      '60': '方壓折帶在塌。秘密部隊說「可控損失」——老子聽見了哭聲。',
      '61': '方工炸區。他們要你快，礦工來不及撤。手別快過腦。',
      '62': '六鑰孔三炸鎖同一條命脈。排錯順序，帳上多一頁名字。',
      '63': '方縱峽夾擊掃。噬星者在峽口，無名部隊在背後推你按鈕。',
      '64': '方熱十——最後十秒，十個還在礦道裡的呼號。排完，別回頭。',
      // 第 9 章：血色共振
      '65': '方鄰焰共振數據對齊。小明，相鄰佈雷會引發波形疊加，我們正在把戰線推回地底。',
      '66': '方偵測到大面積地下空腔。在這裡疊加共振，回音能把噬星者的前鋒部隊耳膜震碎。',
      '67': '六干擾跳變極其劇烈。別被跳動的數字干擾，在脈衝最混亂的交界，點燃我們的火焰。',
      '68': '方地殼結構極不穩定。老張在瘋狂加固鋼樑，我們每引爆一處共振，都在和塌方賽跑。',
      '69': '方噬星者在瘋狂逆流反撲。交錯佈署數字，用疊加的火網把這條交叉廊道焊死！',
      '70': '六這裡是一座巨型噬星者的屍骨巢穴。殘存的能量在自激，穩住節奏，用數學壓制牠。',
      '71': '六讀數在發燙。克萊兒說我們的神經過載快到極限了……撐下去，最後一條地下管線就在前面。',
      '72': '方這是地底核心。小明，把所有的鄰焰連成一條不絕的樂譜，給地底的畜生送葬！',
      // 第 10 章：終焉沙盤
      '73': '方終柵防禦網啟動。小明……這是我最後一次幫你校準頻率了。別看漏任何一個鄰格。',
      '74': '六地形在方格與六角間瘋狂解體。裂隙引力在拉扯，別讓節奏亂掉，把真信號死死釘住。',
      '75': '方通訊節點正在被大量廢雷吞噬！搶下秒數，那是克萊兒用精神安定劑幫我們換來的時間！',
      '76': '方據點遭到連環反撲。老張和鎢鋼在最外層用肉身擋著，數字寫滿，別讓他們的血白流！',
      '77': '方城市的防護罩只剩最後 10%。每一次共振，都是我們在對著深淵扇耳光。堅持住！',
      '78': '六干擾跳變停止了……不，是干擾強大到把頻譜燒黑了。閉上眼睛，相信你的生理反射！',
      '79': '方沙盤盤面只剩下最後的焦點。波比和巴克已經衝進裂隙邊緣去幫你找錨點了……快啊！',
      '80': '方最終核心。小明……最後一手了。把終柵頻率壓下去，接大家回家！',
    },
    missionBriefFallback: [
      '把雜訊當敵人，濾掉就只剩座標。',
      '機率收斂前，別急著下第二手。',
