...
import { hydrateLevelDefinitions, type MapLayoutFilePayload } from './hydrateLevelMaps';
...
export { hydrateLevelDefinitions, type MapLayoutFilePayload } from './hydrateLevelMaps';
...
function mapLayoutFromBundledGlob(mapRef: string): MapLayout {
...
export function parseLevelsJson(raw: unknown): LevelDefinition[] {
  return finalizeLevelDefinitions(hydrateLevelDefinitions(parseLevelsJsonStored(raw), mapLayoutFromBundledGlob));
}
 ...
 * 若關卡使用 `mapRef`，請傳入 `mapLayoutsByRef`（由重載流程 fetch 各 `maps/{ref}.json` 的 `mapLayout`）。
 */
export function applyReloadedLevelDefinitionsJson(
  raw: unknown,
  mapLayoutsByRef?: Record<string, MapLayout>
): void {
  const stored = parseLevelsJsonStored(raw);
  const resolve =
    mapLayoutsByRef != null
      ? (ref: string) => {
          const ml = mapLayoutsByRef[ref];
          if (ml == null) throw new Error(`重載關卡資料時缺少 mapRef「${ref}」對應的 mapLayout`);
          return ml;
        }
      : mapLayoutFromBundledGlob;
